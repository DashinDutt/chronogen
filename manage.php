...
<body style="white-space:nowrap">
  <div id="header">
    ...
    <div id="header_text"><img src="https://minorproject.merchit.in/images/CHRONOGEN.png" style="
    float: left;
    width: 200px;
    padding-top: 10px;
    padding-left: 10px;
"></div>
  ...
  <div id="nav_bar">
    <ul class="main_menu" id="main_menu">
      ...
      <li class="limenu"><a href="manage.php?action=faculty">Manage Batches</a></li>
      <li class="limenu"><a href="manage.php?action=batches">Manage Faculty</a></li>
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="register.php">
          <input type="text" name="fullName" class="styled uInfo" required pattern=".{6,50}" title="6 to 50 characters" placeholder="Batch Name" />
          ...
          <label>Do not use Username as <b>admin</b></label>
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="register.php?action=deleteFaculty" class="confirm">
          <select name="uName" class="updateSelect stretch" data-placeholder="Choose Batch..." required>
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="depts.php?action=add">
          <input type="text" name="dept_code" class="styled details" required pattern="[^ ]{2,6}" title="2 to 5 characters" placeholder="Department Code" />
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="batches.php?action=add">
          <input type="text" name="batch_name" class="styled uInfo" required pattern="[^:]{2,30}" title="Name of Faculty" placeholder="Faculty Name" />
          <select name="dept" class="stretch" data-placeholder="Choose Department..." required>
            <option label="Choose Department..."></option>
            <?php
            foreach($db->query('SELECT * FROM depts') as $dept)
         ...
         <!-- <input type="text" id="bhu" name="size" class="styled details" required pattern="[0-9]{1,3}" title="Number less than 1000, this will be used to suggest rooms" placeholder="Batch Size" />-->
          <!-- <div class="blocktext info"></div>-->
          <div class="center button">
          ...
          <script>
function myFunction() {
  document.getElementById("bhu").defaultValue = "50";
}
</script>
        </form>
      </div>
    ...
    <div class="box">
      ...
      <div class="elements">
        <form method="post" action="batches.php?action=delete" class="confirm">
          <select name="batch" class="updateSelect stretch"  data-placeholder="Choose Faculty..." required>
            <option label="Choose Batch..."></option>
            <?php
            foreach($db->query('SELECT * FROM batches') as $batch)
              echo "<option value=\"{$batch['batch_name']} : {$batch['batch_dept']}\">{$batch['batch_name']} : {$batch['batch_dept']} </option>";
            ?>
          </select>
          <input type="hidden" id="confirm_msg" value="Are you sure you want to delete the selected batch?">
          <div class="blocktext info"></div>
  ...
  <div id="footer">Powered by <a href="https://vashisthbhushan.com">Uttkarsh & Vashisth</a></div>
</body>
</html>
