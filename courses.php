...
if(valueCheck('action','add'))
{
  ...
  try
  {
    $query = $db->prepare('INSERT INTO courses(course_Id,course_name,fac_id,allow_conflict,credits) values (?,?,?,?,?)');
    $query->execute([$cId,$_POST['cName'],$_SESSION['faculty'],$_POST["allowConflict"],$_POST["cCredits"]]);
